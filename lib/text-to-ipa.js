...
(function () {
  ...
  // Load the dictionary from ipadict.js. 
  if (typeof TextToIPA.loadDictByObj !== 'function') {
    TextToIPA.loadDictByObj = function (IPADictObj) {
      console.log('TextToIPA: Loading dict by Object[' + (typeof IPADictObj) + '] ...' );

      if (typeof IPADictObj !== 'object') {
        console.log('TextToIPA Error: IPADictObj is not valid!');
      } else {

        for (var IPAObj of IPADictObj) {
          TextToIPA._IPADict[IPAObj['word']] = IPAObj['ipa'];
        }
  ...
  if (typeof TextToIPA.lookup !== 'function') {
    ...
    TextToIPA.lookup = function (word, opt) {
      opt = Object.assign({
        multi: 1,
      }, opt)
